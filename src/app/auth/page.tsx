...
export default function AuthPage() {
    ...
    try {
      if (tab === "LOGIN") {
        ...
        if (error) {
          ...
          setMessage({ type: "success", text: "Successfully authenticated! Redirecting to Nexus..." });
          setTimeout(() => {
  ...
  const handleGoogleAuth = async () => {
    try {
      ...
      setMessage({ type: "error", text: "Google Authentication initialized." });
    }
  };
  ...
  return (
    <div className="min-h-screen w-full bg-black text-white flex flex-col justify-center items-center p-4 relative overflow-hidden">
      ...
      <Link
        href="/"
        className="absolute top-8 left-8 flex items-center gap-2 z-20 hover:opacity-80 transition-opacity"
      >
      ...
      {/* Centered Minimalist Auth Modal */}
      <motion.div
      ...
      >
        <GlassCard className="p-8 border-purple-500/30 shadow-[0_0_80px_rgba(124,58,237,0.25)] relative overflow-hidden" glowColor="purple">
          ...
          {/* Split Tabs: Student Login / Create Account */}
          ...
          <div className="relative flex items-center justify-center my-4">
            ...
            <span className="bg-zinc-950 px-3 text-[10px] uppercase tracking-wider font-semibold text-zinc-500 absolute">
              or email
            </span>
          </div>
          ...
          {message && (
            <div
              className={cn(
                "p-3 rounded-xl text-xs font-medium mb-4 flex items-center gap-2",
          ...
          <form onSubmit={handleAuth} className="space-y-4">
            ...
            >
              {loading ? "Processing..." : tab === "LOGIN" ? "Sign In to Portal" : "Create Student Account"}
